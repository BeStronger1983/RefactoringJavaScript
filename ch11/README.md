...
如果試圖從一個框架轉換到另一個框架，那算是「重寫」。

另一種重寫是在寫新版應用程式的同時也支援舊版應用程式，直到新版完成度夠高之後才替換舊版，進行起來非常困難，理由如下：

-   兩個不同框架的應用程式的功能等價(feature parity)難以定義。
-   很可能會想為新應用程式改變設計或是加上新功能，但這就超出實現功能等級的範圍了。
-   必須為各種客戶定義出一份堅實(可能是分段式的)的遷移計劃。
-   如果使用新技術，那團隊中那些使用舊技術的專家們，可能沒有足夠的經驗能夠承擔重寫的任務。
-   在舊應用程式中良好運作的「業務規則」可能在翻譯成新應用程式時有所遺漏，或因為臭蟲而有誤。
-   難以呈現進度。
-   很難評估重寫所需要的時間。

重寫並非像重構那樣簡單、機械化的過程。

之前提過的原則：小步改變、並以測試及版本控制來支撐這些改變，這個原則很有用。此外，清晰的說明需要的時間，包括承認估算可能不準確。當有疑慮，就直接將那時間乘二，再不行就乘三。
